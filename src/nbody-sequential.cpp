...
Window window;
GC gc;
int screen, blackcolor, whitecolor, mf, xmin, ymin;
...
int main(int argc, char const **argv)
{
    ...
    bool window = argv[7] == string("enable");
    int len, window_len;
    if (window) {
        xmin = stoi(argv[8]), ymin = stoi(argv[9]);
        len = stoi(argv[10]), window_len = stoi(argv[11]);
        mf = window_len / len;
